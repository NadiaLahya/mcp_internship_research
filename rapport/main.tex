...
\end{console}
    ...
    Le nœud \texttt{IA Agent} analyse les résultats de recherche bruts de Tavily et extrait les informations clés, en utilisant mon prompt personnalisé. Parmi ces informations figurent :
    \begin{multicols}{2}
        \begin{itemize}
            \item le titre de l'offre
            \item l'entreprise
            \item l'URL
            \item la ville
            \item la durée du stage
            \item les technologies requises (skills)
            \item la rémunération proposée
            \item la date limite de candidature
        \end{itemize}
    \end{multicols}

    \subsection{Système de classification : TOP / BOF / NON}
        \subsubsection{Justification du système de verdict}
        Au vu du volume important de résultats de recherche, j'ai implémenté un système de classification tripartite pour filtrer efficacement les opportunités :
        ...
        Une étape importante a été la restructuration des données entre les nœuds grâce aux nœuds \texttt{Format agent input} et \texttt{Format agent output}. 
        
        Premièrement, le nœud \texttt{Format agent input} agit en amont pour préparer les données avant l'analyse du LLM. Il convertit les données brutes et mal stucturées issues du nœud \texttt{Merge},entre Tavily et la récupération du clamment via le MCP, en un format JSON propre et cohérent, facilitant ainsi l'extraction des informations par le LLM.

        Enfin, le nœud \texttt{Format agent output} effectue plusieurs opérations essentielles :
        \begin{enumerate}
...
\section{Résultats}
    ...
    Le système génère automatiquement un rapport formaté pour Discord, qui comporte :
        \begin{itemize}
            ...
            \item un fichier CSV récapitulatif des meilleurs offres trouvées correspondants à ma recherche (consultable \href{https://github.com/NadiaLahya/mcp_internship_research/blob/main/output/internships_offers.csv}{ici})
        \end{itemize}
